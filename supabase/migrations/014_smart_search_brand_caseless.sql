-- ============================================================================
-- Migration 014 — smart_search: brand_filter case-insensitive (LOWER karşılaştırma)
-- ============================================================================
--
-- KÖK NEDEN:
--   Migration 013'te brand_filter exact match: `p.brand = ANY(brand_filter)`.
--   Filter 'LCW' (kullanıcı yazımı) ≠ DB 'Lcw' (Title case) → 0 sonuç.
--   eval1 d4 gift/erkek-giyim-ust + LCW: count=0 (beklenen 6-30).
--
-- ÇÖZÜM:
--   4 CTE'de (vector/specs/keyword/category_browse) brand karşılaştırma:
--     ESKİ: p.brand = ANY(brand_filter)
--     YENİ: LOWER(p.brand) = ANY(SELECT LOWER(b) FROM unnest(brand_filter) b)
--   Türkçe karakter case-folding için LOWER yeterli (Lcwaikiki/lcwaikiki, Apple/apple).
--
-- Migration 013'ün diğer tüm değişiklikleri korunur (DROP overload yok burada
-- çünkü 013 sonrası tek versiyon var; CREATE OR REPLACE function override).
-- ============================================================================
...
BEGIN
  ...
  vector_matches AS (
    ...
    WHERE
      ...
      AND (brand_filter IS NULL OR LOWER(p.brand) = ANY(SELECT LOWER(b) FROM unnest(brand_filter) b))
...
-- GIN index idempotent (Migration 013'te oluşturuldu, burada koruyucu)
...
-- Test
-- ============================================================================
--   SELECT id, title, brand, listing_count, match_source FROM smart_search(
--     query_embedding := array_fill(0::float, ARRAY[768])::vector(768),
--     category_filter := 'erkek-giyim-ust',
--     keyword_patterns := ARRAY['tişört'],
--     brand_filter := ARRAY['LCW'],   -- DB 'Lcw' ile case-insensitive eşleşmeli
--     match_threshold := 0.3,
--     match_count := 10
--   );
-- ============================================================================

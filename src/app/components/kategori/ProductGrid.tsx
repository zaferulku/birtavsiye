...
const products = [
  { emoji: "📱", cat: "Akıllı Telefon", name: "iPhone 16 Pro 256GB Doğal Titanyum", rating: "4.8", reviews: "1.284", price: "74.999", oldPrice: "81.200", shops: 14, badge: "🏆 En Tavsiye", badgeColor: "bg-yellow-700", slug: "iphone-16-pro" },
  { emoji: "📱", cat: "Akıllı Telefon", name: "Samsung Galaxy S25 Ultra 512GB", rating: "4.7", reviews: "2.140", price: "62.999", oldPrice: "78.500", shops: 18, badge: "🔥 Çok Satan", badgeColor: "bg-[#E8460A]", slug: "samsung-galaxy-s25-ultra" },
  { emoji: "🎧", cat: "Kulaklık", name: "Sony WH-1000XM5 ANC Kulaklık", rating: "4.9", reviews: "3.820", price: "8.999", oldPrice: "11.500", shops: 22, badge: "⚡ %22 İndirim", badgeColor: "bg-purple-700", slug: "sony-wh-1000xm5" },
  { emoji: "💻", cat: "Laptop", name: "MacBook Air M3 13\" 8GB 256GB", rating: "4.8", reviews: "940", price: "47.499", oldPrice: "", shops: 12, badge: "✨ Yeni", badgeColor: "bg-green-700", slug: "macbook-air-m3" },
  { emoji: "⌚", cat: "Akıllı Saat", name: "Apple Watch Series 10 46mm GPS", rating: "4.6", reviews: "580", price: "19.999", oldPrice: "", shops: 9, badge: "", badgeColor: "", slug: "apple-watch-series-10" },
  { emoji: "📺", cat: "TV", name: "Samsung 65\" QLED 4K Smart TV 2025", rating: "4.5", reviews: "1.120", price: "34.999", oldPrice: "39.900", shops: 11, badge: "🔥 Popüler", badgeColor: "bg-[#E8460A]", slug: "samsung-65-qled" },
];
    ...
    <div className="flex-1">
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        <div className="flex gap-1">
          <button onClick={() => setView("grid")} className={`px-3 py-2 rounded-lg border text-sm transition-all ${view === "grid" ? "border-[#E8460A] text-[#E8460A]" : "border-[#E8E4DF] text-[#6B6760]"}`}>▦</button>
          <button onClick={() => setView("list")} className={`px-3 py-2 rounded-lg border text-sm transition-all ${view === "list" ? "border-[#E8460A] text-[#E8460A]" : "border-[#E8E4DF] text-[#6B6760]"}`}>☰</button>
        </div>
      </div>
      ...
      <div className="bg-[#0F0E0D] rounded-xl p-4 mb-4 flex items-center gap-4">
        ...
        <button className="bg-[#E8460A] text-white text-xs px-4 py-2 rounded-lg font-medium whitespace-nowrap">İndirimleri Gör →</button>
      </div>

      {view === "grid" && (
        <div className="grid grid-cols-3 gap-4">
          {products.map((p, i) => (
            <Link href={`/urun/${p.slug}`} key={i}>
              <div className="bg-white border border-[#E8E4DF] rounded-2xl overflow-hidden cursor-pointer hover:shadow-lg transition-all">
                <div className="h-40 bg-[#F8F6F2] flex items-center justify-center text-5xl relative">
                  ...
                  <button
                    onClick={(e) => { e.preventDefault(); toggleFav(i); }}
      ...
      {view === "list" && (
        <div className="flex flex-col gap-3">
          {products.map((p, i) => (
            <Link href={`/urun/${p.slug}`} key={i}>
              <div className="bg-white border border-[#E8E4DF] rounded-2xl overflow-hidden flex cursor-pointer hover:shadow-md transition-all">
                <div className="w-32 bg-[#F8F6F2] flex items-center justify-center text-4xl flex-shrink-0">{p.emoji}</div>
      ...
      <div className="flex justify-center gap-2 mt-6">
        {["‹", "1", "2", "3", "...", "48", "›"].map((p, i) => (
          <button key={i} className={`w-9 h-9 rounded-lg border text-sm transition-all ${p === "1" ? "bg-[#E8460A] border-[#E8460A] text-white" : "bg-white border-[#E8E4DF] text-[#6B6760] hover:border-[#E8460A] hover:text-[#E8460A]"}`}>
            {p}
          </button>
        ))}
      </div>

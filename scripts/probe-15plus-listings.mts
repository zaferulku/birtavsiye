...
for (const idPrefix of ids) {
  const { data: p } = await sb.from('products').select('id, title, brand, is_active, is_accessory, model_family, model_code, slug').like('id', `${idPrefix}%`).single();
  if (!p) { console.log(`${idPrefix}: NOT FOUND`); continue; }
  ...
  console.log(`  model_family=${p.model_family} model_code=${p.model_code}`);
  console.log(`  listings: ${lst?.length}`);
